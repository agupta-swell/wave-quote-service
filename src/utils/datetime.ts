...
export const getDaysInMonth = (year: number, month: number) => [31, isLeapYear(year) ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31][month];
...
export const dateAdd = (interval: 'day' | 'year' | 'month', value: number, date: Date) => dayjs(date).add(value, interval).toDate();
