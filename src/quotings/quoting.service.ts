...
import { QUOTING, Quoting } from './quoting.schema';
...
export class QuotingService {
  ...
  async create(quotingDto: CreateQuotingDto): Promise<OperationResult<{ id: string }>> {
    ...
    transformData.polygons.forEach((polygon: any) => {
      polygon.panel = polygon.panel_id;
    });
  ...
  async getDetails(id: string): Promise<OperationResult<QuotingDto>> {
    ...
    const polygons = await Promise.all(
      quoting.polygons.map(async polygon => {
        const panel = await this.solarPanelService.getDetails(polygon.panel);
        delete polygon._id;
        return {
          ...polygon,
          panel_id: polygon.panel,
          panel,
        };
      }),
    );
